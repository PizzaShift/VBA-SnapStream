...
'If the update date is older than current day, and a new update is true, or bypass is true then run the update.
  'If UpdateDate = Workbooks("Toolkit.xlam").Sheets(" ").Range("Z1").Value < Date Then
    ...
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z1").Value = VBA.Now
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z2").Value = TRUE 'NewUpdate
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z3").Value = FALSE 'BypassUpdate
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z4").Value = TRUE 'KeepChanges
    ...
    'Workbooks("Toolkit.xlam").Sheets(" ").Range("Z1").Value = '//Not Updated, keep previous date & time
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z2").Value = FALSE 'NewUpdate
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z3").Value = FALSE 'BypassUpdate
    Workbooks("Toolkit.xlam").Sheets(" ").Range("Z4").Value = FALSE 'KeepChanges
